...
ReturnValue=baAlgorithm(A, r , x, N, D, slen, n_preds)

#训练集数据储存
trainresults=ReturnValue[0]
filename = "TrainResults_BA_HoltWinters.csv"
...
########################################  测试集上的数据储存 ######################################## 
testresults = data1[0]
filename = "TestResults_BA_HoltWinters.csv"
# 打开文件，并使用csv.writer写入数据
with open(filename, mode='w', newline='') as file:
    writer = csv.writer(file)
    # # for row in data2:
