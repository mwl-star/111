...
with open(r'E:\L\Tohoku\tohoku\tohoku_Generation\training data japan3.csv', 'r', newline='') as csvfile2:
    reader2 = csv.reader(csvfile2)
    # Skip the title row（if have）
    next(reader2)
    # Define the index of columns
...
def psoAlgorithm(w, c1 , c2, x, N, D, n_preds):
    ...
    def func(series, x, n_preds):
        ...
        alpha=x[0]
        beta=x[1]
        m1=HoltExp_model(series, alpha, beta, n_preds)
        return m1[0]
...
filename = "TrainResults_PSO_HoltExp.csv"
...
with open(r'E:\L\Tohoku\tohoku\tohoku_Generation\test data japan3.csv', 'r', newline='') as csvfile2:
    reader2 = csv.reader(csvfile2)
    # Skip the title row（if have）
    next(reader2)
    # Define the index of columns
...
########################################  测试集上的数据储存 ######################################## 
testresults = data1[0]
filename = "TestResults_PSO_HoltExp.csv"
# 打开文件，并使用csv.writer写入数据
with open(filename, mode='w', newline='') as file:
    writer = csv.writer(file)
    # # for row in data2:
