...
with open(r'E:\L\Tohoku\tohoku\tohoku_Generation\training data japan1.csv', 'r', newline='') as csvfile:
...
with open(r'E:\L\Tohoku\tohoku\tohoku_Generation\training data japan2.csv', 'r', newline='') as csvfile2:
...
# A=0.5
# r=0.5 
def baAlgorithm(A, r, x, N, D, slen, n_preds):
    def bat_algorithm(func, A, r ,N_pop=len(x), N_gen=500,
        Qmin=0, Qmax=2, d=3, lower=-2, upper=2):
...
def get_fitness(pop1,pop2,pop3,x,D,slen, n_preds):
    ...
    r=pop1[2]
    N2=5
    bafitness1=baAlgorithm(A, r, x2, N2, D, slen, n_preds)
    # print(defitness1[0])
    ...
    r=pop2[2]
    N4=5
    bafitness2=baAlgorithm(A, r, x4, N4, D, slen, n_preds)
    # print(defitness2[0])
    ...
    r=pop3[2]
    N6=5
    bafitness3=baAlgorithm(A, r, x6, N6, D, slen, n_preds)
    # print(defitness3[0])
...
def crossover_and_mutation(pop, CROSSOVER_RATE_MIN, CROSSOVER_RATE_MAX, CROSSOVER_RATE, k, N_GENERATIONS,MUTATION_RATE_MIN,MUTATION_RATE_MAX,MUTATION_RATE):
	new_pop = []
	fitback = get_fitness(pop[0],pop[1],pop[2],x,D,slen, n_preds)
	fitvalue = fitback[0]	
...
if __name__ == "__main__":
    ...
    #训练集数据储存
    trainvalue=print_info(pop[0],pop[1],pop[2],x,D,slen, n_preds)
    trainresults=trainvalue[1]
    filename = "TrainResults_AutoEvo_HoltWinters.csv"
...
############################################# Test Data Set#########################################################
    ...
    ################# Begin Import the train data1(training data japan1.csv) ##################
    with open(r'E:\L\Tohoku\tohoku\tohoku_Generation\test data japan1.csv', 'r', newline='') as csvfile:
        reader = csv.reader(csvfile)
        # Skip the title row（if have）
        next(reader)
        # Define the index of columns
    ...
    ################# Begin Import the train data2(actual data)(training data japan2.csv) ##############
    with open(r'E:\L\Tohoku\tohoku\tohoku_Generation\test data japan2.csv', 'r', newline='') as csvfile2:
        reader2 = csv.reader(csvfile2)
        # Skip the title row（if have）
        next(reader2)
        # Define the index of columns
    ...
    n_preds=24
    # Define the holt-winters model
    def test_holt_model(series, slen, alpha, beta, gamma, n_preds):
        # The holt-winters model 
        def triple_exponential_smoothing(series, slen, alpha, beta, gamma, n_preds):
            result = []
            seasonals = initial_seasonal_components(series, slen)
    ...
    beta =  Plotvalue[1]
    gamma = Plotvalue[2]
    data1 = test_holt_model(series, slen, alpha, beta, gamma, n_preds)

    ########################################  测试集上的数据储存 ######################################## 
    testresults = data1[0]
    filename = "TestResults_AutoEvo_HoltWinters.csv"
    # 打开文件，并使用csv.writer写入数据
    with open(filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        # # for row in data2:
    ...
    ########################################  测试集上的预测值数据 ######################################## 
    data2 = data1[1]
    filename = "AutoEvo_HoltWinters.csv"
    # 打开文件，并使用csv.writer写入数据
    with open(filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        # for row in data2:
