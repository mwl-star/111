...
def SimpleExp_model(series, alpha, n_preds):
    # 读取时间序列数据
    data = pd.read_csv(r'E:\L\Tohoku\tohoku\tohoku_Consumption\training data japan3.csv', parse_dates=['date'], index_col='date')
...
def test_SimpleExp_model(series, alpha, n_preds):
    # 读取时间序列数据
    data = pd.read_csv(r'E:\L\Tohoku\tohoku\tohoku_Consumption\test data japan3.csv', parse_dates=['date'], index_col='date')
